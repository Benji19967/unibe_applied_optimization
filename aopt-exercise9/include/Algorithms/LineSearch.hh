        ...
        * \return the final step t computed by the back-tracking line search */
        ...
        static double backtracking_line_search_newton_with_infeasible_start(Problem *_problem,
            ...
            while (i < 1000)
            {
                ...
                //   r_pri  = A xt - b
                Vec grad_xt(_x.size());
                _problem->eval_gradient(xt, grad_xt);

                Vec rdual = grad_xt + _A.transpose() * nut;
                Vec rpri  = _A * xt - _b;

                double res_new = rdual.squaredNorm() + rpri.squaredNorm();

                if (res_new <= (1.0 - _alpha * t) * _initial_res)
                    break;
