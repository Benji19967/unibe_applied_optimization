...
/*
  ...
  where:
  - Scalar is the scalar type, i.e. the type of the coefficients (int, float,
  double, ...)
  - RowsAtCompileTime and ColsAtCompileTime are the number of rows and columns
  of the matrix as known **at compile time**
  ...
  The RowsAtCompileTime and ColsAtCompileTime template parameters can take the
  special value Dynamic which indicates that the size is unknown at compile
  time, so must be handled as a run-time variable. In Eigen terminology, such a
  size is referred to as a dynamic size; while a size that is known at compile
  time is called a fixed size.
...
TEST(EigenTutorial, VectorInitialization_OnTheFly_column_float) {
  int N;
  std::cout
      << "Could you please set the number of rows for 'vectorNf' variable?"
      << std::endl;
  std::cin >> N;
...
TEST(EigenTutorial, MatrixAssignment_dense) {
  ...
  ASSERT_TRUE(A3x3.sum() == 45);
  ASSERT_TRUE(A3x3.prod() == 362880);

  ASSERT_TRUE(B3x3.sum() == 45);
  ASSERT_TRUE(B3x3.prod() == 362880);
}
...
TEST(EigenTutorial, MatrixAssignment_sparse) {
  ...
  for (int k = 0; k < sparseA.outerSize(); ++k)
    for (SintMat::InnerIterator it(sparseA, k); it; ++it)
      ASSERT_TRUE(it.value() == fullA[it.row()][it.col()]);
}
